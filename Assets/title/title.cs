...
public class title : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        // 【2026-08 隐藏】测试模式按钮（god-mode）——发布版不再对玩家显示。
        // 方法本体保留（EnsureTestModeButton），需要恢复时取消注释下一行即可。
        ...
        // 切换存档按钮（三存档槽，右上角）
        EnsureSaveSlotButton();
    }

    // ───────────────────────── 切换存档按钮（三存档槽）─────────────────────────
    //   主菜单右上角。点开后可在3 个互相独立的存档槽之间切换 ——
    //   玩家想重新开荒时不必删掉原来的存档（见 SaveSlotManager / SaveSlotUI）。
    private GameObject _saveSlotBtnGo;
    private Text       _saveSlotBtnLabel;

    private void EnsureSaveSlotButton()
    {
        if (_saveSlotBtnGo != null)
        {
            _saveSlotBtnGo.SetActive(true);
            RefreshSaveSlotButtonLabel();
            return;
        }
        ...
        var btnGo = new GameObject("__SaveSlotButton",
        ...
        btnGo.GetComponent<Image>().color = new Color(0.14f, 0.16f, 0.26f, 0.92f);

        var btn = btnGo.GetComponent<Button>();
        var colors = btn.colors;
        colors.normalColor      = new Color(1f, 1f, 1f, 1f);
        colors.highlightedColor = new Color(0.85f, 0.9f, 1f, 1f);
        colors.pressedColor     = new Color(0.65f, 0.75f, 1f, 1f);
        btn.colors = colors;

        // 用内置 Text + LegacyRuntime.ttf（自带完整中文，不会像 TMP 那样回退成 □□□）
        ...
        _saveSlotBtnLabel = labelGo.GetComponent<Text>();
        _saveSlotBtnLabel.alignment = TextAnchor.MiddleCenter;
        _saveSlotBtnLabel.fontSize  = 20;
        _saveSlotBtnLabel.color     = Color.white;
        Font f = Resources.GetBuiltinResource<Font>("LegacyRuntime.ttf");
        if (f == null) f = Resources.GetBuiltinResource<Font>("Arial.ttf");
        if (f != null) _saveSlotBtnLabel.font = f;
        ...
        _saveSlotBtnGo = btnGo;
        RefreshSaveSlotButtonLabel();
    }

    private void RefreshSaveSlotButtonLabel()
    {
        if (_saveSlotBtnLabel == null) return;
        _saveSlotBtnLabel.text = $"切换存档\n(当前：存档 {SaveSlotManager.CurrentSlot})";
    }
